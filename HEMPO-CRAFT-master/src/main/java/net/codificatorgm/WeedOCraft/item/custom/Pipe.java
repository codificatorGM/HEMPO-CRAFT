...
public class Pipe extends Item {
    public Pipe(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if (!cond1 && !cond2) {
            ...
            pPlayer.sendSystemMessage(Component.literal("You need a full Flint and Steel to light the Pipe!"));
        }
        if (cond1 && hasFlint) {
            pPlayer.getInventory().removeItem(hempBudSlot, 1);

            pPlayer.playSound(SoundEvents.FIRE_EXTINGUISH, 1, 0.4f);
